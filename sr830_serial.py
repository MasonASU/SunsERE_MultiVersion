# -*- coding: utf-8 -*-
"""
Created on Wed Apr 14 12:39:01 2021
...
class SR830(serial.Serial):
    ...
    def __init__(self, resourceName, **kwargs):
        ...
        self.serial_write("OUTX 0")
        
    def get_x(self):
        return float(self.serial_ask("OUTP?1"))
    def get_r(self):
        return float(self.serial_ask("OUTP?3"))
    
    def serial_write(self,command):
        self.write((command+"\n").encode())
    
    def serial_ask(self,command):
        self.serial_write(command)
        return self.read_until("\r".encode())
    def get_sensitivity_int(self):
        return int(self.serial_ask("SENS?"))
    def get_sensitivity(self):
        return self.SENSITIVITIES[self.get_sensitivity_int()]
    def set_sensitivity(self,sense_int):
        self.serial_write("SENS %d"%sense_int)
    def get_time_constant_int(self):
        return int(self.serial_ask("OFLT?"))
    def get_time_constant(self):
        return self.TIME_CONSTANTS[self.get_time_constant_int()]
    def set_time_constant(self,time_constant_int):
        self.serial_write("OFLT %d"%time_constant_int)

    def auto_gain(self):
        self.serial_write("AGAN")

    def auto_reserve(self):
        self.serial_write("ARSV")

    def auto_phase(self):
        self.serial_write("APHS")
    ...
    def auto_offset(self, channel):
        ...
        self.serial_write("AOFF %d" % channel)
    ...
    def get_scaling(self, channel):
        ...
        if channel not in self.CHANNELS:
            raise ValueError('SR830 channel is invalid')
        channel = self.CHANNELS.index(channel) + 1
        offset, expand = self.serial_ask("OEXP? %d" % channel).split(','.encode())
    ...
    def set_scaling(self, channel, precent, expand=0):
        ...
        self.serial_write("OEXP %i,%.2f,%i" % (channel, precent, expand))
    ...
    def sample_frequency(self):
        ...
        index = int(self.serial_ask("SRAT?"))
        if index == 14:
            return None  # Trigger
        else:
    ...
    def sample_frequency(self, frequency):
        ...
        self.serial_write("SRAT%f" % index)

    def aquireOnTrigger(self, enable=True):
        self.serial_write("TSTR%d" % enable)

    @property
    def reserve(self):
        return SR830.RESERVE_VALUES[int(self.serial_ask("RMOD?"))]
    ...
    def reserve(self, reserve):
        ...
        self.serial_write("RMOD%d" % index)

    def is_out_of_range(self):
        """ Returns True if the magnitude is out of range
        """
        return int(self.serial_ask("LIAS?2")) == 1

    def quick_range(self):
        """ While the magnitude is out of range, increase
        the sensitivity by one setting
        """
        wait_time = self.TIME_CONSTANTS[int(self.serial_ask("OFLT?"))]
        self.serial_write('LIAE 2,1')
        while self.is_out_of_range():
            self.serial_write("SENS%d" % (int(self.serial_ask("SENS?"))+1))
            time.sleep(0.5*wait_time)
            self.serial_write("*CLS")

    @property
    def buffer_count(self):
        query = self.serial_ask("SPTS?")
        if query.count("\n") > 1:
    ...
    def pause_buffer(self):
        self.serial_write("PAUS")

    def start_buffer(self, fast=False):
        if fast:
            self.serial_write("FAST2;STRD")
        else:
            self.serial_write("FAST0;STRD")
    ...
    def reset_buffer(self):
        # Good
        self.serial_write("REST")

    def trigger(self):
        # Good
        self.serial_write("TRIG")
